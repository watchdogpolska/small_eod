    ...
    def validate_feature(self, value):
        ...
        for dictionary, items in groupby(sorted(value, key=attrgetter("dictionary_id")), attrgetter("dictionary")):
            ...
            if length < dictionary.min_items:
                raise serializers.ValidationError("Minimum number of items for {} is {}".format(dictionary, dictionary.min_items))
            if length > dictionary.max_items:
                raise serializers.ValidationError("Maximum number of items for {} is {}".format(dictionary, dictionary.min_items))
        return value
...
class CaseCountSerializer(CaseSerializer):
    ...
    class Meta:
        ...
        fields = CaseSerializer.Meta.fields + [
            "letter_count", "note_count", "event_count"
        ]